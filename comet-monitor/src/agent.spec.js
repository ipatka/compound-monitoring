...
const { getObjectsFromAbi, getEventFromConfig, createMockEventLogs } = require('./test-utils')
...
describe('monitor emitted events', () => {
    describe('handleTransaction', () => {
        ...
        let findingType
        let findingSeverity
        const contractName = 'Comet'
        const validContractAddress = ethers.utils.getAddress('0xc3d688B66703497DAA19211EEdff47f25384cdc3')
        ...
        let validContractSymbol
        const underlyingAddress = '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48'.toLowerCase()
        const decimals = 0

        beforeEach(async () => {
            const { contracts: configContracts } = config
            const { abiFile, events, symbol } = configContracts[contractName]
            validContractSymbol = symbol
            initializeData = {}

            mockCoinGeckoResponse.data = {}

            mockContract.symbol = jest.fn().mockResolvedValueOnce(validContractSymbol)
            mockContract.baseToken = jest.fn().mockResolvedValueOnce(underlyingAddress)
        ...
        it('returns empty findings if contract address does not match', async () => {
            ...
            defaultLog.signature = iface.getEvent(eventInConfig.name).format(ethers.utils.FormatTypes.minimal).substring(6)
        ...
        it('returns empty findings if contract address matches but no monitored event was emitted', async () => {
            ...
            // defaultLog.args = mockArgs;
            defaultLog.data = data
            defaultLog.signature = iface.getEvent(eventNotInConfig.name).format(ethers.utils.FormatTypes.minimal).substring(6)
        ...
        it('returns a finding if a target contract emits a monitored event', async () => {
            ...
            const testFindings = [
                Finding.fromObject({
                    ...
                    description: `📈 - The ${eventInConfig.name} event was emitted by the ${protocolAbbreviation} contract`,
        ...
        it('returns a finding with a whale emoji if a target contract emits a monitored event with a value over 1,000 USD', async () => {
            ...
            console.log({ mockTxEvent, defaultLog })
            const findings = await handleTransaction(mockTxEvent)
            ...
            console.log({ findings, testFindings })
            expect(findings).toStrictEqual(testFindings)
        })
    })
})
